...
# Collect the LaTeX-defined equations in the directory.
$files = Get-ChildItem -Path $dirLatex -File -Filter "*.tex"
# Exclude the LaTeX typeface configuration file.
$files = $files | Where-Object { $_.Name -ne "typeface-config.tex" }
...
foreach ($file in $files) {
    ...
    # Use "[xml]" to cast SVG content to a navigable XML object.
    [xml]$svg = Get-Content $pathSVG

    # Clean up unneeded leading XML tags.
    # N.B. Piping output to "Out-Null" suppresses operation console output.
    if ($svg.FirstChild.NodeType -eq "XmlDeclaration") {
        $svg.RemoveChild($svg.FirstChild) | Out-Null
    }
